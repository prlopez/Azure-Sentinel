...
This playbook enables automated file hash sandbox report lookups in Zscaler Internet Access (ZIA) when triggered by Microsoft Sentinel incidents. It uses OAuth2 authentication to securely query the Zscaler Sandbox API and retrieve the detonation report for file hashes found in incidents.

## Overview

The Zscaler-Oauth2-LookupSandboxReport playbook is designed to:

- Automatically extract FileHash entities from Microsoft Sentinel incidents
- Extract the MD5 hash from each entity
- Authenticate with Zscaler ZIA using OAuth2 via the authentication playbook
- Query Zscaler's sandbox report API to retrieve the full detonation report for each hash

## Prerequisites

Before deploying this playbook, ensure you have:

1. A Zscaler Internet Access (ZIA) subscription with API access and Sandbox enabled
2. The **Zscaler-Oauth2-Authentication** playbook deployed in the same resource group
3. A Logic Apps Integration Account (Free tier) in the same resource group
...
3. **Configure Automation Rules**
   ...
   - Configure rules to run on incidents containing FileHash entities
...
| Zscaler OAuth2 Authentication Playbook | Name of the authentication playbook | Zscaler-Oauth2-Authentication |

## Workflow

1. Triggered by Microsoft Sentinel incident creation
2. Extracts FileHash entities from the incident and reads the MD5 value
3. Calls the Zscaler-Oauth2-Authentication playbook to obtain an access token and the API `base_url`
4. Queries the Zscaler sandbox report API (`/sandbox/report/{md5}?details=full`) using the returned `base_url`
5. Returns the full sandbox detonation report for enrichment
